...
## Vue de score toujours disponible avec état de preuve (0.6.1)

La version 0.5.2 a restauré la formule déterministe v0.4.1 ; la version 0.5.4 a séparé la disponibilité du score de la vérification des citations ; la version 0.5.5 garantit un total de 0 à 100 dès que l’étape 3 produit un rapport ; la version 0.6.0 a remplacé la formule v0.4.1 par `formula 0.5.0-anchored` pour corriger le défaut Base-90 « pas de défaut, donc excellent » ; la version 0.6.1 remplace cette formule par `formula 0.6.0-band-anchored` pour corriger une double pénalisation qu’elle avait introduite — les critères de métier recevaient à la fois le bonus et, en cas d’absence, une nouvelle déduction du même registre de la dimension, si bien qu’un texte compétent mais plat pouvait légitimement échouer à près de la moitié de ses critères et saturer le score au plancher. Le score n’est plus dérivé de façon indépendante : c’est désormais une **projection lisible de la bande littéraire**. La bande retient le plus strict de deux plafonds indépendants — un plafond de métier (la bande A exige que chaque siège central vérifie ≥60 % de son ensemble de métier ; sinon la moyenne des quatre sièges la limite à B ou à « d’archive ») et un plafond de défauts (lignes rouges inchangées : tout problème veto/core limite à B, tout veto de haute sévérité limite à C) — et cette bande fixe une fenêtre de score ; le vecteur de critères ne fait que positionner le texte à l’intérieur. Les sièges n’attribuent toujours aucun nombre : seul `derive_report.py` calcule `derived-report.json.scores`. Les entrées complètes utilisent `status=verified` ; un NA core/extended assorti d’un motif d’inapplicabilité explicite est également un jugement neutre achevé. ABSTAIN, un veto NA, les citations invalides, une exécution dégradée ou des sièges non couverts donnent un score mécanique `status=provisional`, avec chaque limite dans `status_reasons`. Les dimensions absentes restent non évaluées ; lorsque la bande littéraire est N/A, la projection de fenêtre ne s’applique pas et le total retombe sur l’expérience lecteur, la propreté IA puis la fidélité, et sans aucune dimension exploitable, le rapport utilise une base diagnostique fixe de 50.
...
La bande littéraire fixe la fenêtre de score : A/candidate-A `[85,94]`, B (hors archive) `[75,84]`, B (d’archive) `[68,74]`, C `[45,59]`. La bande mécanique plafonne à A ; S (canonique) ne peut être distingué de A par le seul vecteur de critères (les deux se situent à 87–100 % de métier) — c’est une décision humaine de conservation, jamais un résultat de la formule. La position dans la fenêtre dépend du `craft_ratio` de chaque siège central (proportion vérifiée de son **ensemble de critères positifs de métier** : 04 : N3/TW2/TW4/SC1, 05 : P2/P3/P4/P7, 06 : L5/L7/TW3, 07 : E3/E6/E7/TW14), combinée à une densité de défauts pondérée par sévérité pour les problèmes hors métier ; les quatre dimensions centrales partagent la fenêtre du total et restent comparables entre elles. **Les critères de métier n’ajoutent désormais que de la preuve positive et ne sont plus jamais déduits pour leur absence** — c’est la correction sémantique centrale de cette version. La propreté IA (base 100), l’expérience lecteur (base 85) et la fidélité (A=90/B=65/C=45) conservent leurs propres échelles absolues à titre purement diagnostique ; l’originalité n’ajoute jamais que (+5 %, +3 % ou +0 %, jamais de retrait). Le plafond de fidélité s’applique en dernier, après la projection de fenêtre : la fidélité B limite le total à 75, la fidélité C à 45 — un contrat existant qui peut légitimement pousser le score hors de la fenêtre de sa bande. Toutes les constantes des deux plafonds (les seuils de 60 %/30 %, la pondération 0,7/0,3 entre moyenne et maillon faible, et les quatre fenêtres) sont marquées **provisoires**, calibrées à partir d’une revue en direct de 18 textes le 2026-08-26, et sujettes à recalibration future. Les sièges 04-07 émettent aussi une comparaison ancrée contre `band-{a,b,c}.md` dont une divergence ≥2 rangs part en arbitrage humain. Les classes de la vue 0-100 vont de A à D. **Les scores sont mécaniquement dérivés des vecteurs de critères ; les sièges ne produisent aucun nombre.**
