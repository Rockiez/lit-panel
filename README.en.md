...
`prepare_run.py` accepts `--genre memoir|other` (default `memoir`) and `--readers=N` (default 1). The default `standard` preset enables Seats 01-09 and 11. `--source` activates fidelity Seat 01, while `--brief` activates editorial-intent Seat 10. The base `quick` set is 01, 02, 03, and 08, but memoir runs automatically add ethics Seat 11; because the literary core is absent, a closed quick run has literary band N/A. `full` covers Seats 01-11, while missing source or brief inputs are disclosed as coverage gaps. A memoir `custom(...)` run that explicitly excludes Seat 11 also creates a warning gap.
