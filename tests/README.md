...
2. **机械核验复算**（如 `verify_quotes.py`）：对一次真机执行产出的判据表，独立重放 `SKILL.md` §4 的 Tier 1–5 分层核验算法（含精确匹配、归一化、省略号跨度、Tier 4 模糊对齐非通过仲裁候选生成与"注毒测试"——故意篡改一条引文验证核验管道真的会拦下捏造引文），确认阶段二核验机制本身可信，不是评审会话自己说"我核验过了"就采信。
...
如果你想在自己的评审报告上做同样的机械核验复核，使用随包分发的通用版本 `<skill-dir>/scripts/verify-quotes.py`（仓库根目录 `scripts/verify-quotes.py` 保持兼容入口）——它搭载了完整的 Tier 1–5 分层核验引擎（Tier 1 Exact、Tier 2 Normalized、Tier 3 Span Ellipsis 带有片段长度约束、Tier 4 Fuzzy Alignment 仅作为非通过仲裁候选、Tier 5 Void/Skip），不含任何真实引文数据，所有输入（被评文本、来源素材、待核验的引文列表）都通过命令行参数传入（推荐显式指定 `--max-tier 5`）：

```bash
python3 <skill-dir>/scripts/verify-quotes.py <quotes.json> <被评文本路径> --max-tier 5 [--source <来源素材路径或目录>] [--format text|markdown|json] [--fuzzy-threshold 0.85]
```
