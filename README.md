...
- 文学带决定分数窗口：A/A候选 `[85,94]`、B(非记录型) `[75,84]`、B(记录型) `[68,74]`、C `[45,59]`；机械带位封顶于 A，S（传世）无法由判据向量与 A 区分（两者 craft 比例均在 87%–100%），是人工典藏裁定，不由公式产生。
...
项目禁止评审席直接打分、主观百分比和自由加权；只允许闭合脚本按 `0.6.0-band-anchored` 冻结公式导出可复算的 0-100 `scores`。A/B/C/N/A 仍是独立的质性带位；作废引文不能进入正式带位合成，并会使该轮失去正式闭合资格，但对应冻结判定仍进入暂定评分，且必须披露证据状态。
