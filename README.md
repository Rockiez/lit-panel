...
- **席11 的边界**：席11 审的是文本内**呈现方式**是否有伦理风险（单方转述定性、隐私必要性、错误归因、弱者尊严），不替代出版前针对真实人物的场外授权/同意检查——文本内呈现得体，不代表已经取得相关人物的发布同意，这一步仍需人工在文本之外完成。
- **引证机械核验的防线边界**：逐字核验只防"捏造引文"（原文里根本查无此句），不防"引真文说假话"（引文确实存在，但 note 里的断言与其实际含义不符）——后者不是机械核验能力所及，由互盲多席交叉与人工仲裁承接，报告使用者应知晓这道防线止步于此。

## 已知风险

- **判据公开是一把双刃剑（Goodhart 定律）**：`skills/lit-panel/references/criteria/*.md` 全部随包公开分发——这既是本工具"可审计、可复核、可反驳"的基础，也意味着如果生成侧模型被专门训练/提示去"应付"这些具体判据句，理论上能在不真正提升文本质量的情况下让判据表看起来更漂亮（一旦指标本身被当成优化目标，它就会失去指示价值）。**素读者（席08）与各席"自由观点"字段是天然的抗应试面**——素读者读前完全看不到判据，自由观点不受判据束缚，两者都不是能被"背题"绕过的固定考点。
- **进阶缓解：自建私有判据**。如果你需要恢复"暗考题"效果（生成方无法提前得知的额外判据），可以自建 `skills/lit-panel/references/criteria/99-private.md`（或类似命名），把它加入 `.gitignore`（不随仓库分发、不进入公开的判据池），并按 `registry.md`「扩席说明」的操作步骤把它挂载为一个新席位，或并入既有席位的判据文件。这类私有判据不改变本仓库分发的公开契约，是你本地环境的可选加固——lit-panel 本身不强制、不预设任何私有判据内容。
