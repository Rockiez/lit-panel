...
La bande littéraire fixe la fenêtre de score, et l'échelle est une **partition complète de `[0,100]` sans trous** : `S` `[95,100]`, A/candidate-A `[80,94]`, B (hors archive) `[68,79]`, B (d'archive) `[60,67]`, C `[40,59]`, et `0-39` est réservé à la zone d'échec de fidélité (produite uniquement par la bande de fidélité C). La position dans une bande est normalisée par rapport à la plage atteignable de cette bande, si bien que chaque fenêtre est pleinement utilisée : la bande A est passée de 3 scores entiers atteignables en 0.6 à 15. `S` (canonique) n'est toujours pas produit par le vecteur de critères (il ne peut pas séparer S de A ; les deux se situent à 87–100 % de métier) ; il est promu par le canal de comparaison ancrée : les deux plafonds à A, **aucune alerte de volonté de partage du lecteur naïf**, et les quatre sièges centraux plaçant le texte à « 接近S » face à `band-s.md`. Si l'une de ces conditions manque, l'exécution reste en A.
La fidélité s'applique en dernier : la fidélité B plafonne le total à 60 (le plancher de la bande B) ; la fidélité C **projette** dans la zone d'échec `[0,39]` au lieu de plafonner — un plafond ne produirait qu'un seul point et laisserait tout ce qui est en dessous sans propriétaire. La propreté IA, l'expérience lecteur et la fidélité **n'émettent plus de scores 0-100** ; elles rapportent des lectures natives (occurrences détectées / lecteurs et problèmes / bande de fidélité). Elles ne sont pas sur l'échelle des bandes et ne sont pas comparables aux quatre dimensions littéraires. Les classes de la vue 0-100 réutilisent désormais les noms de bande au lieu d'un vocabulaire A–D séparé.
