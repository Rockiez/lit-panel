...
## 始终可用、带证据状态的评分视图（0.6.1）

0.5.2 把 v0.4.1 的确定性评分公式接回闭合运行时；0.5.4 将“能否评分”和“引文是否核实”解耦；0.5.5 进一步保证阶段三成功生成的每份报告都有 0-100 数值总分；0.6.0 以 `formula 0.5.0-anchored` 取代 v0.4.1 公式，修复 Base-90「无错即优」；0.6.1 以 `formula 0.7.0-full-partition` 取代 0.6.0 公式，修复其埋下的双重惩罚——craft 判据此前既靠加分又被同一维度的扣分账本二次扣分，致使一篇称职但平实的文本合法未通过近半判据、分数触底饱和。分数不再独立导出，而是**文学带的可读投影**：文学带取两条独立天花板中更严格者——craft 天花板（每核心席 craft_ratio ≥60% 才封顶 A，否则看四席均值封顶 B 或「记录型」）与缺陷天花板（沿用既有红线，任一 veto/core 问题封顶 B，高严重度 veto 封顶 C）——决定一个分数窗口，判据向量只负责把文本定位在窗口内部。评审席仍然零打分，只提交判据向量；只有 `derive_report.py` 可以生成 `derived-report.json.scores`。完整且已核验的输入使用 `status=verified`；有明确适用性理由的 core/extended NA 同样视为完整判定。ABSTAIN、veto NA、引文作废、降级执行或未覆盖席位继续按已有判据机械导分，但标记 `status=provisional` 并在 `status_reasons` 逐项披露。缺少的维度显示为未评测，不伪装成已覆盖；文学带为 N/A 时不适用窗口投影，总分回退到读者体验、AI 洁净度、忠实度，完全没有可用评分维度时使用固定诊断基线 50。

未提供 source 时，只把忠实度显示为“未评测”并令 `dimensions.fidelity=null`，其他维度和总分照常形成；提供 source 后，即使忠实席引文错误，也会根据原席冻结判定形成暂定忠实度分。引文作废仍会阻断正式 A/B/C 带位、红线和修订结论，暂定评分绝不冒充已核实证据。

- 文学带决定分数窗口，阶梯是 `[0,100]` 的**完整划分、不留空洞**：`S` `[95,100]`、A/A候选 `[80,94]`、B(非记录型) `[68,79]`、B(记录型) `[60,67]`、C `[40,59]`，`0-39` 是忠实失败区（只由忠实带 C 产生）。带内定位按各带自身的可达区间归一化，因此每个窗口都被用满——A 带从 0.6 时代的 3 个可达整数分扩到 15 个。`S`（传世）不由判据向量产生（它无法把 S 与 A 分开，两者 craft 比例均在 87%–100%），改由锚定对照通道升带：缺陷与 craft 两条天花板均为 A、**无素读者传播意愿预警**、且四个核心席对照 `band-s.md` 一致给出「接近S」时升 S；任一条不满足即停在 A。
- 忠实度最后应用：忠实 B 把总分钳位到 60（B 带下界）；忠实 C **投影**进忠实失败区 `[0,39]`（不再钳位——钳位只产出一个点，会让 0 到该点之间无人认领）。AI 洁净度、素读者体验、忠实度三个诊断维度**不再给 0-100 分**，改报原生读数（检出处数 / 读者人数与不适处数 / 忠实带）；它们不在带位阶梯上，与四个文学维度不可比。0-100 视图的档位标签直接复用带位名，不再是独立的 A–D 词表。
- 窗口内定位由每个核心席（04/05/06/07）的 `craft_ratio`（**正向工艺判据集** 04：N3/TW2/TW4/SC1，05：P2/P3/P4/P7，06：L5/L7/TW3，07：E3/E6/E7/TW14 的已核验 YES 比例）与非 craft 问题的严重度加权缺陷密度共同决定；四个核心维度分与总分同窗口、彼此可比。**craft 成员从此只加分，不再因缺席被二次扣分**——这是本版对上一版双重惩罚的核心修正。
- AI 洁净度、读者体验、忠实度三个维度保持独立的绝对刻度（基准分别为 100、85、按忠实带 A=90/B=65/C=45），仅供诊断参照。原创性只加不减：O2/O3/O5/O6 全部通过且无 O 系问题加 5%，至少三项通过且无 O 系问题加 3%，否则不加。
- 忠实度封顶在窗口投影之后最后应用：忠实 B 把总分封顶为 75，忠实 C 封顶为 45——这是既有合同，允许总分合法跨出所在带位窗口。
- 文学带的 craft 天花板与缺陷天花板全部常数（0.6/0.3 门槛、0.7/0.3 均值短板权重、四个窗口）均标注为 **provisional**，来自 2026-08-26 的 18 篇活体校准，将随后续校准更新。席 04–07 另对照 `references/anchors/band-{a,b,c}.md` 输出锚定对比带位，与机导带位偏差 ≥2 档进入人工仲裁。
...
项目禁止评审席直接打分、主观百分比和自由加权；只允许闭合脚本按 `0.7.0-full-partition` 冻结公式导出可复算的 0-100 `scores`。A/B/C/N/A 仍是独立的质性带位；作废引文不能进入正式带位合成，并会使该轮失去正式闭合资格，但对应冻结判定仍进入暂定评分，且必须披露证据状态。
