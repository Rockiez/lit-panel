# lit-panel 设计规格（同步至 v0.4.1 — 构建期参考文件）
...
| 09 | originality-reviewer.md | lit-originality | criteria/09-originality.md | 原创性与陈套：人写烂的套路（区别于席3的AI套话）、意外而合理、个性色彩 | 总是 | 加分维（v0.4.1起不入带位；问题判定降为打磨建议） | — |
...
3. **带位规则**：文学带——席04/05/06/07（v0.4.1 起席09原创性退出本带位，改走下方规则8"原创加分"）的 core 判据分 veto/普通两层（veto=各席至多2条最致命判据，逐席清单见 `criteria/CHANGELOG.md` v0.2.0 节，09 的 veto 身份已于 v0.4.1 撤销）：veto 问题判定且高严重度 → 封顶 C；veto 问题判定但中/低严重度 → 最高 B 且转人工仲裁；普通 core 任一问题判定 → 最高 B；core 全过（零问题判定）→ A 候选（结合 anchors 对照）。素读者不参与 A 候选判定本身，改为报警器：core 全过时若素读者传播意愿答"不愿意" → 强制人工仲裁，不自动发 A（细则见 SKILL.md §5.3）。忠实带——席01 五态分布（CONTRADICTED 或高严重度 UNSUPPORTED 存在 → C；仅低严重度问题 → B；干净 → A）；无 source 时记 N/A。
...
8. **评分导出**（v0.4.0 新增，v0.4.1 加入原创加分，细则见 SKILL.md §5.8）：判据向量的公开公式机械导出视图，不是评审席判断，评审席契约不变。文学四维（结构/人物/语言/情感）各基准90，veto问题判定按严重度封顶（高≤45/中低≤65），普通core每条问题判定−12，extended每条−5，零问题判定且锚点对照确认可至95-100；AI洁净度（席03，基准100，−3/条上限−10）、读者体验（素读者R系列，基准85，−10/条）、忠实度（有source时取自忠实带字母：C=45/B=65/A=90）三个补充维度各自独立公式。**原创加分**（v0.4.1新增，席09不入带位、仅加分）：O系通过类判据（O2/O3/O5/O6）全部YES且零问题判定→+5，≥3条YES且零问题判定→+3，其余（含任何问题判定）→+0，任何情况下不扣分。总分=文学四维简单平均，叠加席03修正（−3/条，上限−10）、素读者报警旁标（不扣分）、原创加分（只加不减，上限100）、忠实带总分封顶（C→封顶45且决策强制为"重写建议"；B→封顶75，均在加分之后仍生效）。等级映射：A=90+/A-=85-89/B+=80-84/B=70-79/C+=60-69/C=45-59/D<45。报告分数区域强制附一行脚注："分数由判据向量机械导出，评审席不产生任何数字。"
