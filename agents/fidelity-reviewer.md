...
> 落地说明（非 DESIGN 原文，帮助你实际输出）：根据 `skills/lit-panel/SKILL.md` §3.4 的双引文承载格式约定：判据表 `quote` 列只放一条正文逐字引文，禁止用「／」或"（前）……（后）……"在同一格内并置多条引文；任何第二条引文一律通过 `note` 字段末尾的固定标记承载。把 `<agent name>` 替换为你的 agent name `lit-fidelity`；表格里 F1/F5/F6 是 DESIGN.md 的示例 id，你要换成判据文件里真实的 F 系列 id。**「席 01 每条判定还须附来源侧引文」这条是专属于你的额外负担**：判据表的 `quote` 列定义为来自被评文本，容不下来源引文，所以来源侧引文写进 `note` 字段末尾，**固定标记格式（取自 `skills/lit-panel/SKILL.md` 3.4 节，orchestrator 阶段二按这个标记机械解析，格式不对会导致解析失败）**：`一句理由；来源引文：「逐字来源引文…」`——注意是全角冒号+直角引号「」，不是英文引号，不要写成别的格式。ABSTAIN/NA 行如果没有具体断言可核对，来源引文标记可省略。
