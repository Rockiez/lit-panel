...
- The literary band sets the score window, and the ladder is a **complete partition of `[0,100]` with no gaps**: `S` `[95,100]`, A/A-candidate `[80,94]`, B (non-record-grade) `[68,79]`, B (record-grade) `[60,67]`, C `[40,59]`, with `0-39` reserved for the fidelity-failure zone (produced only by fidelity band C). Position inside a band is normalised against that band's own reachable range, so every window is fully used — band A went from 3 reachable integer scores under 0.6 to 15. `S` (canonical) is still not produced by the criterion vector (it cannot separate S from A; both sit at 87–100% craft ratio); it is promoted through the anchor-comparison channel instead: both ceilings at A, **no naive-reader sharing warning**, and all four core seats independently placing the text at 接近S against `band-s.md`. Any one of those failing leaves the run at A.
- Fidelity applies last: fidelity B clamps the total to 60 (band B's floor); fidelity C **projects** into the fidelity-failure zone `[0,39]` rather than clamping — a clamp would produce a single point and leave everything below it unclaimed. AI cleanliness, naive-reader experience and fidelity **no longer emit 0-100 dimension scores**; they report native readings (findings detected / readers and problems / fidelity band). They are not on the band ladder and are not comparable with the four literary dimensions. Grade labels in the 0-100 view now reuse the band names instead of a separate A–D vocabulary.
...
Every YES/NO judgment requires a verbatim quote. Schema validation and mechanical quote verification occur before synthesis; a failed quote invalidates the criterion as formal evidence, prevents formal closure for that run, and cannot affect a formal band. It does not erase the numeric view: the frozen judgment contributes to a clearly marked provisional score. Seats may not assign scores, percentages, or free-form weights; only the closed script may derive reproducible 0-100 `scores` with the frozen `0.7.0-full-partition` formula. A/B/C/N/A remains an independent qualitative view.
