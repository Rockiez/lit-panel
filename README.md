...
阶段一 · 互盲多智能体并行审读 —— 十一席各自基于物理隔离上下文独立阅读，
        ...
        │
        ▼
阶段二 · 逐字引证机械核验 —— 针对报告全部 quote 字段执行机械复核；
                          无法在原文定位的引证判定强制即刻作废（防幻觉注入机制）
        │
        ▼
...
- **已获真机完备验证**：多智能体互盲分发管道、逐字引文机械检索核验与伪证熔断机制、`quick`/`standard` 双预设装配、跨家族异质模型互评、忠实链全流程、Veto 三层带位降级算法、素读者两步法追问回收。
