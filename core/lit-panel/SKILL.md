...
### 7.1 评分导出（冻结公式 0.7.0-full-partition）

评分不再独立导出，而是**第 7 节第 3 条已判定文学带的可读投影**：带位（已被 18 篇活体评审 + 跨模型家族验证，见提交记录）决定一个分数窗口，判据向量只负责把文本定位在窗口内部，不再自行决定量纲起点。闭合变体标识为 `formula_version=0.7.0-full-partition`；报告 schema 1.2 使用 `scores.status` 与 `status_reasons` 披露证据状态。**只要阶段三成功生成报告，就必须输出 0-100 数值总分，不得因 ABSTAIN、NA、引文作废、降级执行、缺少 source、preset 未覆盖全部评分席或局部席位失败而清空分数。** 完整且已核验的评分输入使用 `status=verified`；有明确适用性理由的 core/extended NA 视为完整判定，不扣分、不降低评分状态。ABSTAIN、veto NA 或其他覆盖/执行/引文问题使用 `status=provisional`，并逐项列明原因。文学带非 `N/A` 时，04–07 已按定义完整覆盖，四个核心席的窗口内定位系数均可求得；文学带为 `N/A`（未完整覆盖 04/05/06/07）时不适用窗口投影，总分依次回退到读者体验、AI 洁净度、忠实度，完全没有可用评分维度时使用固定诊断基线 50；**回退总分恒封顶 79，不得进入 `A`/`S` 区间**——两者的准入前提是四个文学席到齐，而回退启动的条件恰恰是它们缺席。这些代理量只在内部充当量纲，不再作为维度分对外呈现。正式带位仍由完整证据门禁独立控制。
...
1. **分数窗口** `[lo, hi]` 由文学带决定，阶梯是 `[0,100]` 的**完整划分，不留空洞**：`S` → `[95, 100]`；`A`/`A候选（待人工确认）` → `[80, 94]`；`B`（非记录型） → `[68, 79]`；`B`（记录型） → `[60, 67]`；`C` → `[40, 59]`；`0–39` 是忠实失败区，只由忠实带 `C` 产生；`N/A` 不适用窗口投影，见上段回退链。
   `S`（传世）**不由判据向量产生**——判据向量无法区分 `S` 与 `A`（两者四个核心席 `craft_ratio_i` 均落在 0.87–1.00 区间）。`S` 由锚定对照通道升带：缺陷天花板与 craft 天花板均为 `A`、**无素读者传播意愿预警**、且四个核心席的 `anchor_comparison.placement` 一致为 `接近S` 时升 `S`；任一条不满足即停在 `A`（或 `A候选`），分歧进人工仲裁区。预警一票否决升 S：「传世」的字面意思就是会被传下去，素读者答「不愿意把这篇讲给别人听」与之构成定义矛盾。
...
5. **忠实度在窗口投影之后最后应用**：忠实 `B` 把总分钳位到 60（`B` 带下界）；忠实 `C` **不再钳位，而是把文学定位投影进忠实失败区 `[0, 39]`**。改投影的理由是钳位只会产出一个点，`0` 到该点之间将无人认领；投影让整段有主，且写得好的出局与写得烂的出局仍然可分（定位 0.90 → 35.1，0.10 → 3.9）。一篇因忠实度出局的文本，其窗口内定位仍然真实。
...
「干净文学维度」这一提法本身被 0.7.0-full-partition 取代——分数不再有独立的维度基准，只有带位窗口内的位置。v0.4.1 历史文档中的 95–100 “历史锚点上浮”没有闭合运行输入，0.5.2 不允许 orchestrator 主观决定是否上浮，0.7.0-full-partition 同样不允许：`S` 只能由四席一致的锚定对照产生，orchestrator 不得自行判定。报告必须原样保留：**分数由判据向量机械导出，评审席不产生任何数字。**
