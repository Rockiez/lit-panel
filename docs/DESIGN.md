...
**预设分档**：`quick`=01,02,03,08；`standard`=01–08；`full`=01–11；`custom`=注册表任选。条件激活席在条件不满足时自动跳过并在报告中注明。
...
3. **带位规则**：文学带——席 04/05/06/07/09 的 core 判据任一 NO → 封顶 C；core 全过且席08体验报告正向 → A 候选（结合 anchors 对照）；其余 B。忠实带——席01 五态分布（CONTRADICTED 或高严重度 UNSUPPORTED 存在 → C；仅低严重度问题 → B；干净 → A）；无 source 时记 N/A。
4. **分歧区**：席位间对同一文本区域结论相反 → 并列呈现双方判定+引文，**不平均、不裁决**。
5. **人工仲裁区**：全部 ABSTAIN + 席11 全部发现 + 作废判定。
6. **决策建议**：按矩阵（忠实优先）：忠实A×文学A=交付；忠实C 或 文学C=重写建议；其余=修订后交付。席10 fail 直接追加修订项。输出为**建议**，含终止提示（建议最多 2 轮重写后转人工）。
7. **修订包**：每条 NO 的 id+引文+严重度+修改建议，可直接喂给修订会话。
