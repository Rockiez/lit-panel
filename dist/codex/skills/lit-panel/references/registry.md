...
| 09 | originality-reviewer.md | lit-originality | criteria/09-originality.md | 原创性与陈套：人写烂的套路（区别于席3的AI套话）、意外而合理、个性色彩 | 总是 | 质性旁证，不入带位 | — |
...
- **agent name**：subagent frontmatter 的 `name` 字段；三端 adapter 用它生成或寻址原生 Agent。插件单独运行且没有预注册 custom agent 时，把对应 persona 注入通用 subagent。
...
| 预设 | 席位集合 | 说明 |
|---|---|---|
| `quick` | 01, 02, 03, 08 | 最小可用集：来源忠实 + 内部一致 + AI 痕迹 + 素读者体验。不含文学带核心席（04/05/06/07），因此文学带为 N/A；也不含席 09 的原创性质性旁证。 |
| `standard` | 01–09 + 11 | 默认预设。覆盖忠实（条件激活）、一致性、AI 痕迹、四席文学核心**全部**（04/05/06/07）、原创性旁证（09）、素读者、他者与伦理，不含 10；若提供 `--brief`，10 自动并入。 |
