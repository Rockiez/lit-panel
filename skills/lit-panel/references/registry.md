...
| 预设 | 席位集合 | 说明 |
|---|---|---|
| `quick` | 01, 02, 03, 08 | 最小可用集：来源忠实 + 内部一致 + AI 痕迹 + 素读者体验。不含任何"文学带核心席"（04/05/06/07），因此 quick 预设下文学带规则中的 core 判据集合为空——此时文学带不判定 A/B/C，只在报告中记"quick 预设未覆盖文学带核心判据，本轮不产出文学带"。（09 原创性加分维同样不在 quick 内，本轮不产出原创加分。） |
| `standard` | 01–09 + 11 | 默认预设。覆盖忠实（条件激活）、一致性、AI 痕迹、四席文学核心**全部**（04/05/06/07）、原创加分维（09，v0.4.1起不入带位、仅贡献总分加分）、素读者、他者与伦理（回忆录默认开启），不含 10；若提供 `--brief`，10 自动并入本轮激活集合，不需要改用 `custom` 显式列出。 |
