...
**Herramienta de verificación de citas independiente**: Para auditar de forma autónoma la autenticidad de las citas fuera de las sesiones, ejecute el script de verificación jerárquico Tier 1–5 incluido: `python3 <skill-dir>/scripts/verify-quotes.py quotes.json capitulo.md --max-tier 5 [--source entrevista.md] [--format text|markdown|json] [--fuzzy-threshold 0.85] [--include-tier]` (predeterminado `--max-tier 1` solo para coincidencia exacta; la Etapa 2 recomienda explícitamente `--max-tier 5`; Tier 4 genera candidatos de arbitraje no aprobados separados de Tier 5 void; la salida JSON declara `schema_version` `lit-panel.quote-verification/v1`).
